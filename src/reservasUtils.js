...
/** Uma entrada de auditoria `cancelamento` → objeto de reserva (última versão por id é escolhida pelo chamador). */
function reservationFromAuditCancelEntry(e) {
  if (!e || e.tipo !== 'cancelamento') return null
  ...
  return { car, id, row, at }
}

function mergeLatestAuditRowsById(log, predicate) {
  const latestById = new Map()
  for (const e of log) {
    const meta = reservationFromAuditCancelEntry(e)
    if (!meta) continue
    if (!predicate(meta)) continue
    const prev = latestById.get(meta.id)
    if (!prev || meta.at > String(prev._auditAt || '')) {
      latestById.set(meta.id, { ...meta.row, _auditAt: meta.at })
    }
  }
...
export function auditCancellationsAsReservationsForDay(dateISO, { carOnly = false } = {}) {
  ...
  return mergeLatestAuditRowsById(log, (meta) => {
    if (carOnly !== meta.car) return false
    return reservationCoversDate(meta.row, dateISO)
  })
}

/**
 * Todos os cancelamentos da auditoria local (qualquer data), deduplicados por id da reserva.
 * `carOnly`: true = só carro; false = só salas.
 */
export function auditAllCancellationsAsReservations({ carOnly = false } = {}) {
  const log = readAuditLog()
  return mergeLatestAuditRowsById(log, (meta) => carOnly === meta.car)
}
